...
- **AI Providers** (header + sidebar panel) are real, passive availability
  ...
  both are real dispatch backends now. Readiness is surfaced everywhere an
  agent could be dispatched blind into a known-broken provider, not just
  after the fact: the Agent Registry, Dispatch, and Fleet's orchestrator
  picker and refine button all show an inline warning when the relevant
  provider isn't `ready` — added after a real dispatch failed with a raw
  `spawn claude ENOENT` and the only way to learn why was reading server
  logs. `streamClaudeCodeChat` (`src/lib/providers/claude-code.ts`) also
  turns that specific failure into an actionable message pointing at
  `CLAUDE_CODE_BIN` instead of the raw Node error.
- **Agents vs. Goals.** An **Agent** is a reusable *specialist* — a "who"
  (Code Specialist, Quality Reviewer), defined once by what it's good at and
  assigned to any number of goals over time. A **Goal** is one specific
  *deliverable* — a "what" ("Verify the totals and percentage math"),
  belonging to one project. Keeping these distinct is load-bearing: an
  earlier version of Fleet built both from the same task title/description,
  so every agent was an exact clone of its goal and the registry filled with
  single-use, task-named rows. Materialization now assigns work to existing
  agents and creates none.
- **Agent Registry** (`/agents`) is real local data with full read/write API
  ...
  existing one inline, delete one you no longer need. Both `ollama/<model>`
  and `claude-code/<model>` agents actually run when dispatched. Deleting an
  agent also strips it from any goal's assignment list, so nothing is left
  pointing at a row that no longer exists. **Pause** benches an agent: it's
  filtered out of every dispatch picker, out of the orchestrator's roster,
  and refused server-side with a clear message — not a cosmetic label.
- **Goals** (`/goals`) is a three-column board across the app's three domains
  ...
  delete, set priority, and assign agents from the registry — the
...
- **Status reflects reality.** Goal and agent status used to be write-only
  fields that only ever held whatever was last clicked — a goal whose every
  task had finished still read "Not Started". Dispatch now advances both
  (goal → In Progress → Done/Blocked, agent → Active → Idle), and every page
  additionally derives live status from run history via `deriveGoalStatus` /
  `deriveAgentStatus` in `src/lib/status.ts`, which is also the single source
  of status labels and colors app-wide (previously eight near-identical maps
  across seven components, already drifted enough that one page said "Not
  Started" and another `not-started`).
...
- **Fleet** (`/fleet`) is the end-to-end pipeline: blurb a rough idea,
  ...
  Goals assigned to your existing agents, then dispatch each and watch it complete on an org-chart
  ...
  Tasks can depend on each other (e.g. a review task depending on what it
  reviews) — the orchestrator proposes this automatically where it makes
  sense, you can adjust it via toggle chips before materializing, and a
  dependent task's prompt automatically gets the upstream task's completed
  output as context once dispatched. Confirmed live with a real 4-task
  pipeline (draft → fact-check → edit → title, each later stage depending
  on the ones before it) generated by an actual Claude Code CLI
  orchestrator. The orchestrator is given your actual agent roster (name,
  role, model) and assigns each task to the specialist that fits it, by
  name. Without that roster it was choosing blind: the prompt named two
  example models inline, so it tagged nearly every task with the "smart"
  example and once emitted a model tag that wasn't even installed.
  Confirmed live — a script-writing brain dump now fans out to four
  different specialists (Code Specialist → write it, Math Specialist →
  verify the math, Quick Tasks → README, Quality Reviewer → review pass)
  instead of one model four times. A task the orchestrator can't match
  materializes **unassigned** rather than inventing an agent; you pick one
  during review or later from the Goals page. A "Copy deliverables"
  button bundles every completed task's output into one paste-ready block.
  **Scope note:** this produces a finished, copy-ready deliverable — it
  does not post anywhere. There's no YouTube (or other platform)
  integration; publishing is something you do yourself with the output.
  See `/api/projects` and [Architecture](#architecture).
...
priority into a system+user prompt — plus, when the goal has
`dependsOnGoalIds` (see Fleet below), each dependency's latest *complete*
run's output, resolved by the route and passed in as an optional fourth
argument. A dependency with no completed run yet is just left out; dispatch
is never blocked server-side waiting on one — the org chart is what tells
the user a task is waiting, not a gate. `providerIdForModel`
(`src/lib/providers/types.ts`) is the one place that maps an agent's model
prefix to a provider id — a small `dispatchAgent` helper in
[`src/lib/runs.ts`](src/lib/runs.ts) uses it to pick the provider — so
nothing else about the route's shape changes when a new provider is added:
...
- `POST /api/projects/[id]/plan` takes the (possibly user-edited) task
  list and materializes it: an existing agent is only reused on an exact
  match of *both* `model` and `role` — a new one gets created otherwise.
  Matching on model alone was tried first and broke live: a "Quality
  Control" agent's role bled into an unrelated "write a draft" task
  because both happened to suggest the same model tag, and the dispatched
  agent's own response flagged the mismatch it had been handed. Role
  comes from the task description, so in practice each distinct task gets
  its own agent — correctness over deduplication, since agents are cheap
  to create and the Agent Registry already supports rename/delete cleanup.
  Each task also becomes a `Goal` with `domain: "content"` and
  `projectId` set, and — if it declared `dependsOn` — `dependsOnGoalIds`,
  resolved from sibling task titles once every goal in the plan exists.
...
- The org chart itself does zero new data-fetching beyond what already
  ...
  by `projectId`, matching the scale this is meant to run at. A task card
  shows "Depends on: …" (sibling goal titles, resolved client-side from
  `dependsOnGoalIds`) and a "waiting on deps" status distinct from "not
  dispatched" when a dependency hasn't completed yet — informational only,
  dispatch is still a normal link into Runs either way.
- Dependencies flow through as **titles** while still a draft
  (`ProposedTask.dependsOn`, since goal ids don't exist yet) and become
  **ids** once real (`Goal.dependsOnGoalIds`). Renaming a task in the
  review step propagates the rename into any sibling's `dependsOn`
  referencing the old title, so editing before materializing doesn't
  silently break a reference; removing a task drops it from any sibling's
  list the same way. Either path can still leave an unresolvable title if
  edited in a way the client-side propagation didn't cover — the plan
  route just drops those rather than erroring.
...
a response with one malformed task mixed into otherwise-valid ones (and
`dependsOn` parsing specifically: present, absent, non-string entries
dropped) — the malformed-task case is what an imperfect real model
response actually looks like, not a hypothetical. `buildRunPrompt` is
tested both with and without upstream context, and
`providerIdForModel`/`agentProviderStatus` (`src/lib/providers/types.ts`)
have their own small suite.
...
src/
  ...
  lib/providers/types.ts      Shared types + providerIdForModel/agentProviderStatus (unit tested)
