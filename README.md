...
- **Fleet** (`/fleet`) is the end-to-end pipeline: blurb a rough idea,
  ...
  orchestrator. The model field in task review is a guided picker built
  from your existing agents' models (plus Claude Code, if reachable) —
  free text is still possible via an explicit "Custom…" option, with a
  visible warning when the orchestrator's own suggestion doesn't match
  anything real, which is exactly the failure mode this closes: a small
  local model once proposed task "models" like `claude-video/edit2.0` that
  don't correspond to any real provider, and nothing caught it before
  those became real, silently-broken Agent rows. A "Copy deliverables"
